...
# Removed h_type and set True directly
if Config.ZELZAL_A:
    ...
    async def install():
        ...
        try:
            ...
            LOGS.error(f"Error fetching full channel info: {e}")
        ...
        for module in range(total):
            ...
            if plugin_name.endswith(".py"):
                ...
                try:
                    ...
                    LOGS.error(f"Failed to download plugin {plugin_name}: {e}")
                    continue  # Continue processing other plugins

        print(inst_done)  # Ensure inst_done is properly defined
        addgvar("PMLOG", True)  # Directly use True for the boolean value
        if gvarstatus("GRPLOG") is not None:  # Fixed typo GRPLOG
            addgvar("GRPLOG", True)  # Set GRPLOG to True

    zedub.loop.create_task(install())  # Ensure this runs in the event loop
